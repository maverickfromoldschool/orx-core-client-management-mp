...
```js
  <orx-core-pricing id="orx-core-pricing" text="Hello World" />
...
export default function Home() {
  ...
  return (
    <div>
      <Script src="http://127.0.0.1:4173/orx-core-pricing.umd.js" strategy="afterInteractive" />
      <orx-core-pricing ref={ref} id="orx-core-pricing" text="Home Web Component" />
    </div>
  );
}
```
...
declare global {
  ...
  namespace JSX {
    interface IntrinsicElements {
      'orx-core-pricing':OrxCorePricingProps;
    }
  }
}

interface OrxCorePricingProps extends React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> {
  text: string;
}
```
...
import {OrxCorePricingEvent, ROUTE_EVENT, ANALYTICS_EVENT, ERROR_EVENT} from '@optum-rx-core/orx-core-pricing';
...
export function useOrxCorePricingRef(): UseOrxCorePricingRefReturn {
  ...
  const handleEvent = ((e: CustomEvent<OrxCorePricingEvent>) => {
    const {detail} = e;
...
```js
  const element = document.getElementById('orx-core-pricing-webcomponent');
  element.addEventListener('event', (e: CustomEvent<OrxCorePricingEvent>) => {
    const {detail} = e;
...
```js
  ...
  <orx-core-pricing id="orx-core-pricing-webcomponent" text="Hello World" route-action="goToRoute" analytics-action="trackAnalytics" error-action="handleError" theme="optumTheme" />
  <script src="http://path/to/cdn/file.umd.js"></script>
```
