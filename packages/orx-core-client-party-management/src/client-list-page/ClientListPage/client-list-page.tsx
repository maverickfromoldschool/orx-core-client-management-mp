...
export function ClientListPage(props: ClientListPageProps) {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      ...
      try {
        ...
        const mappedClients: Client[] = (response.message.data || []).map((apiClient) => {
          return {
            clientId: apiClient.party.partyIdentifier,
            draftId: '',
            // draftId :apiClient.draftId,
            clientName: apiClient.organization.organizationName.organizatioName,

            status: 'Complete',
            clientReferenceId: apiClient.party.partyIdentifiers[0]?.value || '',
            effectiveDate: '',
            operationalUnitsCount: 0
          };
        });

        setClients(mappedClients);
        setTotalClients(response.message.totalRecord || 0);
        setTotalPages(response.message.totalPages || 0);
        setIsLoading(false);
      } catch (err) {
        setIsLoading(false);
  ...
  const handleClientClick = (client: Client) => {
    navigate(`/edit-client?mode=draft&clientId=${client.clientId}&draftId=${client.draftId}`);
  };
