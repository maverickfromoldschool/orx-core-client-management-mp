...
import {apiClient as cagApiClient} from '@optum-rx-core/orx-core-axios';
...
export const cagsApiService = {
  ...
  getCAGsPaged: async ({page = 0, size = 10, params = {}}): Promise<any> => {
    try {
      const response = await cagApiClient.get('/api/cag/getUnassignedCagList', {
  ...
  getCAGById: async (cagId: string): Promise<CAG | null> => {
    try {
      const response = await cagApiClient.get(`/api/cags/${cagId}`);
  ...
  getClients: async (): Promise<Client[]> => {
    try {
      ...
      const response = await cagApiClient.get('/api/clients/activeClientList');
      // eslint-disable-next-line no-console
  ...
  getContractsByClientId: async (clientId: string): Promise<unknown[]> => {
    try {
      ...
      const response = await cagApiClient.get<{contractList?: unknown[]}>('/api/clients/contractList', {
        params: {clientId}
      });
      // eslint-disable-next-line no-console
  ...
  getOperationalUnitsByClientAndContract: async (contractInternalId: string): Promise<any[]> => {
    try {
      // Use cagApiClient for consistent API calls
      const response = await cagApiClient.get<{operationUnitList?: any[]}>('/api/clients/activeOperationUnitList', {
  ...
  getAssignedCagList: async ({
    ...
    try {
      ...
      // Use cagApiClient for consistent API calls
      const response = await cagApiClient.get('/api/cag/assignedCAGList', {
        params
      });
      // eslint-disable-next-line no-console
  ...
  createCAG: async (data: CAGCreateRequest): Promise<CAG> => {
    try {
      const response = await cagApiClient.post('/api/cags', data);
  ...
  assignCAGs: async (data: {
    ...
    try {
      ...
      console.log('Request URL:', '/api/cag/assign');

      const response = await cagApiClient.post('/api/cag/assign', data, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
  ...
  editAssignedCAG: async (data: {
    ...
    try {
      ...
      console.log('Request URL:', '/api/cag/editAssignedCAG');

      const response = await cagApiClient.post('/api/cag/editAssignedCAG', data, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
  ...
  updateCAG: async (cagId: string, data: CAGUpdateRequest): Promise<CAG> => {
    try {
      const response = await cagApiClient.put(`/api/cags/${cagId}`, data);
      return response.data as CAG;
    } catch (error) {
      return handleApiError(error as AxiosError);
    }
