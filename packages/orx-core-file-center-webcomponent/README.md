...
```js
  <orx-core-file-center id="orx-core-file-center" text="Hello World" />
...
export default function Home() {
  ...
  return (
    <div>
      <Script src="http://127.0.0.1:4173/orx-core-file-center.umd.js" strategy="afterInteractive" />
      <orx-core-file-center ref={ref} id="orx-core-file-center" text="Home Web Component" />
    </div>
  );
}
```
...
declare global {
  ...
  namespace JSX {
    interface IntrinsicElements {
      'orx-core-file-center':OrxCoreFileCenterProps;
    }
  }
}

interface OrxCoreFileCenterProps extends React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> {
  text: string;
}
```
...
import {OrxCoreFileCenterEvent, ROUTE_EVENT, ANALYTICS_EVENT, ERROR_EVENT} from '@optum-rx-core/orx-core-file-center';

import {UseOrxCoreFileCenterRefReturn} from './use-orx-core-file-center-ref.types';

export function useOrxCoreFileCenterRef(): UseOrxCoreFileCenterRefReturn {
  const ref = React.useRef<HTMLDivElement>(null);
  const {push} = useRouter();

  const handleEvent = ((e: CustomEvent<OrxCoreFileCenterEvent>) => {
    const {detail} = e;
...
export default useOrxCoreFileCenterRef;
...
```js
  const element = document.getElementById('orx-core-file-center-webcomponent');
  element.addEventListener('event', (e: CustomEvent<OrxCoreFileCenterEvent>) => {
    const {detail} = e;
...
```js
  ...
  <orx-core-file-center id="orx-core-file-center-webcomponent" text="Hello World" route-action="goToRoute" analytics-action="trackAnalytics" error-action="handleError" theme="optumTheme" />
  <script src="http://path/to/cdn/file.umd.js"></script>
```
