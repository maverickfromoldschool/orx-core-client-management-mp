...
```js
  <orx-core-products id="orx-core-products" text="Hello World" />
...
export default function Home() {
  ...
  return (
    <div>
      <Script src="http://127.0.0.1:4173/orx-core-products.umd.js" strategy="afterInteractive" />
      <orx-core-products ref={ref} id="orx-core-products" text="Home Web Component" />
    </div>
  );
}
```
...
declare global {
  ...
  namespace JSX {
    interface IntrinsicElements {
      'orx-core-products':OrxCoreProductsProps;
    }
  }
}

interface OrxCoreProductsProps extends React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> {
  text: string;
}
```
...
import {OrxCoreProductsEvent, ROUTE_EVENT, ANALYTICS_EVENT, ERROR_EVENT} from '@optum-rx-core/orx-core-products';
...
export function useOrxCoreProductsRef(): UseOrxCoreProductsRefReturn {
  ...
  const handleEvent = ((e: CustomEvent<OrxCoreProductsEvent>) => {
    const {detail} = e;
...
```js
  const element = document.getElementById('orx-core-products-webcomponent');
  element.addEventListener('event', (e: CustomEvent<OrxCoreProductsEvent>) => {
    const {detail} = e;
...
```js
  ...
  <orx-core-products id="orx-core-products-webcomponent" text="Hello World" route-action="goToRoute" analytics-action="trackAnalytics" error-action="handleError" theme="optumTheme" />
  <script src="http://path/to/cdn/file.umd.js"></script>
```
